...
###### Devem ser colocadas aqui as identifica��es das interrup��es e exce��es ###
	...
	csrrwi zero,66,1	# caso ocorra dropdown vai gerar exce��o de instru��o inv�lida
...
errorExceptions: csrrw 	s11, 67, zero 	# le o utval da exce��o e salva em s11	
...
ecallException:   addi    sp, sp, -264              # Salva todos os registradores na pilha
	...
	goToPrintIntUnsigned: jal     printIntUnsigned	# chama printIntUnsigned
		j	endEcall
...
loop1printIntUnsigned:	div 	t4, a0, t2			# divide por 10 (quociente)
			...
			mv 	a0, t4				# atualiza o numero com o quociente
			addi 	t1, t1, 1			# incrementa o contador de digitos
			bne 	a0, zero, loop1printIntUnsigned		# verifica se o numero eh zero
				
loop2printIntUnsigned:	lw 	t2, 0(sp)			# le digito da pilha
			addi 	sp, sp, 4			# libera espaco
			addi 	t2, t2, 48			# converte o digito para ascii
			sb 	t2, 0(t0)			# coloca caractere no buffer
			addi 	t0, t0, 1			# incrementa endereco do buffer
